...
import InstitucionModal from "../../../../components/dashboard/settings/catalogos/InstitucionModal";
...
import {
  ...
  eliminarInstitucion,
} from "../../../../services/dashboard-services/settings/catalogos/institucionesService";
...
const Instituciones = () => {
  ...
  const fetchInstituciones = useCallback(async () => {
    ...
    setInstituciones(data.resultados || []);
  ...
  const handleAbrirEditar = (institucion) => {
    setModoModal("editar");
    setNombreEditar(institucion.nombre_institucion);
    setIdEditar(institucion.id_institucion);
    setModalAbierto(true);
  };
  ...
  const handleGuardar = async (data) => {
    try {
      if (modoModal === "crear") {
        ...
        await actualizarInstitucion(idEditar, data);
  ...
  const handleEliminar = async () => {
    try {
      await eliminarInstitucion(institucionAEliminar);
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {loading ? (
          <p className="text-gray-500">Cargando instituciones...</p>
        ) : (
            ...
            <table className="min-w-full text-sm text-left text-gray-700">
              ...
              <tbody>
                {instituciones.map((institucion, index) => (
                  <tr key={institucion.id_institucion} className="border-b">
                    <td className="px-6 py-4">
                      {(paginaActual - 1) * porPagina + index + 1}
                    </td>
                    <td className="px-6 py-4">{institucion.nombre_institucion}</td>
                    <td className="px-6 py-4 text-right space-x-2">
                      <button
                        className="text-blue-600 hover:text-blue-800"
                        onClick={() => handleAbrirEditar(institucion)}
                      >
                      ...
                      <button
                        ...
                        onClick={() => {
                          setInstitucionAEliminar(institucion.id_institucion);
      ...
      <ConfirmacionModal
        ...
        mensaje="¿Estás seguro de eliminar esta institución académica?"
      />
    </DashboardLayout>
  );
};
