...
import NivelInglesModal from "../../../../components/dashboard/settings/catalogos/NivelInglesModal";
...
import {
  ...
  eliminarNivelIngles,
} from "../../../../services/dashboard-services/settings/catalogos/nivelInglesService";
...
const NivelesIngles = () => {
  ...
  const fetchNiveles = useCallback(async () => {
    setLoading(true);
    const data = await getNivelesIngles({
  ...
  const handleAbrirEditar = (nivel) => {
    setModoModal("editar");
    setNombreEditar(nivel.nivel);
    setIdEditar(nivel.id_nivel_ingles);
    setModalAbierto(true);
  };
  ...
  const handleGuardar = async (data) => {
    try {
      if (modoModal === "crear") {
        await crearNivelIngles(data);
        toast.success("Nivel creado correctamente");
      } else {
        await actualizarNivelIngles(idEditar, data);
  ...
  const handleEliminar = async () => {
    try {
      await eliminarNivelIngles(nivelAEliminar);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Gestión de Niveles de Inglés</h1>
            <Link
            ...
            <table className="min-w-full text-sm text-left text-gray-700">
              ...
              <tbody>
                {niveles.map((nivel, index) => (
                  <tr key={nivel.id_nivel_ingles} className="border-b">
                    <td className="px-6 py-4">
                      {(paginaActual - 1) * porPagina + index + 1}
                    </td>
                    <td className="px-6 py-4">{nivel.nivel}</td>
                    <td className="px-6 py-4 text-right space-x-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          setNivelAEliminar(nivel.id_nivel_ingles);
      ...
      <ConfirmacionModal
        ...
        mensaje="¿Estás seguro de eliminar este nivel de inglés?"
      />
    </DashboardLayout>
  );
};
